...
The most common way that such a user of electricity would offer a product to the electricity market is through a "load drop". This is when an electricity customer reduces their load in exchange for a payment from the system operator. One challenge is that when a user reduces electricity usage, the couterfactual remains unknown. How much electricity would the user have been consuming if they hadn't dropped reduced their load. 

In order to calculate payments, utilities will calculate a "baseline" and the difference between the baseline and the actual usage will be the amount of the actual load drop. The most common way that utilities calculate baselines is known as a 10-of-10 baseline. This is the average hourly load of the last 10 weekdays. Calculating this baseline at any point in time for a user with a historical load data feed will be the central functionality of the package.
...
In addition, there are increasing pathways for distribution level assets to actively participate in wholesale electricity markets. FERC Order 745, filed in 2011 and upheld in the Supreme Court in 2016 requires that Independent System Operators design market rules enabling demand response resources to receive compensation at LMP ($/MWh) for load reduction products.
...
The use case would be for an electricity user that is participating in the wholesale market, to be able to quickly calculate their exact or expected baseline and enable a more informed decision about offering a load drop product.
